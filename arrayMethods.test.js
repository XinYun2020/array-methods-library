...
it("forEach", () => {
  ...
  forEach(startingArray, func); // testing function

  expect(func).toHaveBeenNthCalledWith(1, "a", 0, startingArray);
  expect(func).toHaveBeenNthCalledWith(2, "b", 1, startingArray);
  expect(func).toHaveBeenNthCalledWith(3, "c", 2, startingArray); // expected #ofCall,array[i],i,array
  expect(func).toHaveBeenCalledTimes(3); // number of times function been called
});

it("map", () => {
  const func = jest.fn((elem, index) => index * 2); // map element index * 2 to an array
  const startingArray = ["a", "b", "c"];
  const newArray = map(startingArray, func); // testing function
...
it("filter", () => {
  ...
  const startingArray = ["a", "b", "c"];
  const newArray = filter(startingArray, func); // testing function
...
describe("reduce", () => {
  it("with a starting value", () => {
    ...
    const total = reduce(startingArray, func, 4); // testing function
